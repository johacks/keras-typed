...
from tkeras._core import backend

GLOBAL_STATE_TRACKER = threading.local()
GLOBAL_SETTINGS_TRACKER = threading.local()


def set_global_attribute(name: str, value: Any) -> None:
...
def clear_session(free_memory: bool = True) -> None:
    ...
    for _ in range(100):
        ...
        model = keras.Sequential([keras.layers.Dense(10) for _ in range(10)])
