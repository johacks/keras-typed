...
KerasWeakDType = Literal["int", "float"]
WEAK_TYPES: tuple[KerasWeakDType, ...] = get_args(KerasWeakDType)
KerasComplexDType = Literal["complex64", "complex128"]
COMPLEX_TYPES: tuple[KerasComplexDType, ...] = get_args(KerasComplexDType)
...
KerasFloat8DType = Literal["float8_e4m3fn", "float8_e5m2"]
FLOAT8_TYPES: tuple[KerasFloat8DType, ...] = get_args(KerasFloat8DType)

# All supported dtypes in Keras
KerasAllowedDtype = Literal[
    "float16",
    "float32",
    "float64",
    "uint8",
...
ALLOWED_DTYPES: tuple[KerasAllowedDtype, ...] = get_args(KerasAllowedDtype)
PYTHON_DTYPES_MAP = {
    bool: "bool",
...
def _least_upper_bound(*nodes: KerasAllowedDtype) -> KerasAllowedDtype:
...
def _dtype_and_weaktype(value: Any) -> tuple[KerasAllowedDtype, bool]:
...
def _respect_weak_type(
    dtype: KerasAllowedDtype, weak_type: bool
) -> Union[KerasAllowedDtype, KerasWeakDType, Literal["complex"]]:
...
def _lattice_result_type(*args) -> KerasAllowedDtype:
    dtypes: list[KerasAllowedDtype]
    weak_types: list[bool]
...
def result_type(*dtypes) -> KerasAllowedDtype:
