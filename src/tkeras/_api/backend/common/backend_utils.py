...
from typing import Literal, Optional, SupportsIndex, TypeVar, Union
...
# Type aliases
T = TypeVar("T")
KerasPaddingType = Literal["valid", "same"]
BackendTensor = Shaped[Array, ""]
TOneOrManyTensors = TypeVar(
    "OneOrManyTensors",
    bound=Union[
        BackendTensor, Union[tuple[BackendTensor, ...], list[BackendTensor]]
    ],
)
...
def compute_conv_transpose_padding_args_for_jax(
    ...
    padding: KerasPaddingType,
    output_padding: Optional[Union[int, Sequence[int]]],
...
def compute_conv_transpose_output_shape(
    ...
    data_format: KerasImageDataFormat = "channels_last",
...
def standardize_axis_for_numpy(
...
def to_tuple_or_list(
    value: Optional[Union[int, tuple[int, ...], list[int]]],
) -> Optional[tuple[int, ...]]:
...
def _vectorize_update_dim_sizes(
...
def _vectorize_parse_input_dimensions(
    args: Sequence[BackendTensor],
    input_core_dims: Sequence[Sequence[int]],
...
def _vectorize_check_output_dims(
    func: Callable[[T], TOneOrManyTensors],
    dim_sizes: Union[Iterable[tuple[int, int]], Mapping[int, int]],
    expected_output_core_dims: Optional[Sequence[Sequence[int]]],
) -> Callable[[T], TOneOrManyTensors]:
    from keras.src import ops
...
def _vectorize_apply_excluded(
    func: Callable[..., T],
    excluded: Iterable[Union[int, str]],
    args: tuple,
    kwargs: dict,
) -> tuple[Callable[..., T], tuple, dict]:
    if not excluded:
...
def vectorize_impl(
    pyfunc: Callable[..., TOneOrManyTensors],
    vmap_fn: Callable[
        [Callable[..., TOneOrManyTensors], Sequence[Optional[int]]],
        Callable[..., TOneOrManyTensors],
    ],
    *,
